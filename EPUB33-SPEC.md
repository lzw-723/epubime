...
本解析库的开发将严格遵循并实现对 **EPUB 3.3** 版本的完整支持。EPUB 3.3是W3C于**2023年5月25日**正式发布的推荐标准（W3C Recommendation），是EPUB 3系列规范的最新稳定版本 。选择EPUB 3.3作为目标版本，是因为它不仅代表了当前数字出版领域的最新技术标准和行业共识，而且相较于之前的版本（如EPUB 3.2），它在技术细节上进行了重要的澄清、完善和标准化，特别是在国际化、安全性、隐私保护和可访问性方面 。一个关键的特性是，**EPUB 3.3与EPUB 3.2保持了向后兼容**，这意味着任何有效的EPUB 3.2文件也都是有效的EPUB 3.3文件 。因此，本库在实现对EPUB 3.3新特性的支持的同时，也天然具备了对EPUB 3.2文件的解析能力，从而覆盖了市面上绝大多数的EPUB出版物。
...
*   **不支持的规范特性**：本解析库明确只支持EPUB 3.3。如果解析库遇到为EPUB 2.0或未来版本设计的特性（如某些已弃用的元素或属性），应记录警告并忽略这些不支持的特性，而不是尝试解析它们，以保证库的稳定性。
