...
public class ZipUtilsTest {
    ...
    public void processZipFileContentWithTraversalPathShouldThrowException() throws IOException {
        // Test that using directory traversal path should throw exception
        File zipFile = ResUtils.getFileFromRes("fun/lzwi/epubime/epub/《坟》鲁迅.epub");
        ZipUtils.processZipFileContent(zipFile, "../../../etc/passwd", new Consumer<InputStream>() {
            @Override
            public void accept(InputStream inputStream) {
    ...
    @Test(expected = IOException.class)
    public void getZipFileInputStreamWithTraversalPathShouldThrowException() throws IOException {
        // Test that using directory traversal path should throw exception
        File zipFile = ResUtils.getFileFromRes("fun/lzwi/epubime/epub/《坟》鲁迅.epub");
    ...
    @Test(expected = IOException.class)
    public void getMultipleZipFileContentsWithTraversalPathShouldThrowException() throws IOException {
    ...
    @Test(expected = IOException.class)
    public void getMultipleZipFileBytesWithTraversalPathShouldThrowException() throws IOException {
    ...
    public void processHtmlContent() throws IOException {
        ...
        ZipUtils.processHtmlContent(zipFile, "mimetype", new Consumer<InputStream>() {
            @Override
            public void accept(InputStream inputStream) {
                try {
                    BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream, "UTF-8"));
        ...
        assertTrue("HTML content should be processed", processed[0]);
        assertEquals("application/epub+zip", content.toString());
    }
    
    @Test(expected = IOException.class)
    public void processHtmlContentWithTraversalPathShouldThrowException() throws IOException {
        // Test streaming processing of HTML content using directory traversal path should throw exception
        File zipFile = ResUtils.getFileFromRes("fun/lzwi/epubime/epub/《坟》鲁迅.epub");
        ZipUtils.processHtmlContent(zipFile, "../../../etc/passwd", new Consumer<InputStream>() {
            @Override
            public void accept(InputStream inputStream) {
                // Should not reach here
                fail("Should have thrown IOException");
    ...
    public void processMultipleHtmlContents() throws IOException {
        ...
        ZipUtils.processMultipleHtmlContents(zipFile, filePaths, new BiConsumer<String, InputStream>() {
            @Override
        ...
        assertEquals("Should process 2 files", 2, processedCount[0]);
        assertTrue(contents.containsKey("mimetype"));
        assertTrue(contents.containsKey("META-INF/container.xml"));
        assertEquals("application/epub+zip", contents.get("mimetype"));
        assertNotNull(contents.get("META-INF/container.xml"));
    ...
    @Test(expected = IOException.class)
    public void processMultipleHtmlContentsWithTraversalPathShouldThrowException() throws IOException {
